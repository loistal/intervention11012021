...
#  TODO verfigy and keep as exact copy of individual payslips
#  perhaps move shared code to a module
...
if ($all_employeeid > 0)
{
  ...
  and payslip.employeeid=?
  order by employeename';
  $query_prm = array($all_employeeid);
}
else
{
  $query = 'select payslipid,employeename from payslip,employee
  where payslip.employeeid=employee.employeeid
  and status=? and payslipdate=?
  order by employeename';
  $query_prm = array($status,d_builddate(1,$month,$year));
}
require('inc/doquery.php');
$main_result = $query_result; $num_results_main = $num_results;

for ($xyz=0; $xyz < $num_results_main; $xyz++)
{
  $payslipid = $main_result[$xyz]['payslipid'];
  ...
  if ($exitdate == '' || $exitdate == '0000-00-00' || $payslipdate <= $exitdate) {
  
  if ($xyz>0) { echo '<p class=breakhere></p>'; }
  
  echo '<table class="transparent" width=98%><tr><td width=80% valign=top>
  <span class="sliptitle">Bulletin de Paie'; if ($status == 0) { echo ' - NON VALIDÉ'; }
  ...
  if ($_SESSION['ds_payroll_startday'])
  {
    ...
    $show_payslipdate = d_builddate($_SESSION['ds_payroll_startday'],$lastmonth,$lastyear);
  ...
  echo '</span><td align=right valign=top>';
  ...
  echo '<table class="transparent" width=98%><tr><td width=44% valign=top>';
  ...
  echo '<td width=5%>&nbsp;<td width=49% valign=top>';
  ...
  echo '<table class=report width=96%><tr>';
  echo '<td>Congés Payés acquis avant ce mois: ',$vacationdays_last;
  echo '<td>Congés Payés acquis ce mois: ',$vacationdays_added;
  echo '<tr><td>Congés Payés utilisés ce mois: ',$vacationdays_used;
  echo '<td style="font-weight: bold">Solde Congés Payés: ',$vacationdays;
  echo '</table>';
  ###


  echo '<table class=report width=96% style="border-collapse: collapse">
  ...
  if($hourly_pay == 0)
  {
    ...
    else { echo d_td_old("Salaire de base",0,2),d_td('&nbsp;'.$hourspermonth.' heures par mois','center'); }
  ...
  echo '<br><br><span STYLE="text-align: right; width: 99%"><img src="pics/logo.png" height="50"></span>';
  echo '</table>';
  }
}

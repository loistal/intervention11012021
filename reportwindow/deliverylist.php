...
$query = 'select invoiceid,invoicehistory.clientid,clientname,deliverydate,localvesselid,townname,islandname,outerisland,userid,deliverytypeid
from invoicehistory,client,town,island where client.townid=town.townid and town.islandid=island.islandid and invoicehistory.clientid=client.clientid';
...
for ($i=0; $i < $num_results_main; $i++)
{
  ...
  if ($pallet_exit != 1) { echo '<td><b>DLV'; }
	...
	$query = 'select enteredpurchasebatchid,invoiceitemid,regroupnumber,quantity,unittypename,productname,product.productid as productid,suppliercode,weight,volume,numberperunit,displaymultiplier,netweightlabel,invoiceitemhistory.currentpurchasebatchid
  from invoiceitemhistory,product,unittype,regulationtype
  where product.regulationtypeid=regulationtype.regulationtypeid and invoiceitemhistory.productid=product.productid and product.unittypeid=unittype.unittypeid
  and invoiceid=?';
	if ($_SESSION['ds_customname'] == 'Wing Chong') { $query = $query . ' and regroupnumber<=8'; } # TODO excludefromdelivery
	$query = $query . ' order by regroupnumber asc,invoiceitemhistory.productid,invoiceitemid';
    ...
    if ($amountcarton > 0)
    {
      ...
      if ($form)
      {
        if (isset($_POST['supplierbatchname'.$invoiceitemid]) && $_POST['supplierbatchname'.$invoiceitemid] != '')
        {
          ...
          if ($num_results)
          {
            $query = 'update invoiceitemhistory set enteredpurchasebatchid=? where invoiceitemid=?';

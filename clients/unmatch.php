...
switch($currentstep)
{
  ...
  <td><?php echo $row['matchingid']; ?></td></tr>
  <tr><td>Par:</td>
  <td><?php echo $row['name']; ?></td></tr>
  <tr><td>Date:</td>
  <td><?php echo datefix($row['date']); ?></td></tr>
  <tr><td colspan="2" align="center">
  ...
  $query_prm = array($_POST['matchingid']);
  require('inc/doquery.php');
  ...
  $query_prm = array($_POST['matchingid']);
  require('inc/doquery.php');
  echo '<h2>Délettrer:</h2>';
  echo '<p>Lettrage numéro ' . $_POST['matchingid'] . ' a été de-lettré.</p>';
  break;

...
echo '<h2>Import employé jurion 2018 12 16</h2>';
...
if ($_POST['importme'] == 1)
{
  ...
  while ($line=fgets($fp))
  {
    ...
    $sex = $lineA[8]; #n
    $dn = $lineA[9]; #n
    $familysituation = $lineA[10]; #n id
    $numchildren = $lineA[11]; #n
    $geoaddress = $lineA[12];
    $postaladdress1 = $lineA[13];
    $postalcode = $lineA[14];
    $townid = $lineA[15];
    
    $employeedepartmentid = 0; # employee [lookup id from text]
    $employeesectionid = 0; # employee [lookup id from text]
    $jobid = 0; # employee [lookup id from text]
...
/*
    ...
    */
    ...
    if ($error == 0 && $i>1)
    {
      ...
      $query = 'insert into employee (hourspermonth,basesalary,referencenumber,employeename,employeefirstname,employeedepartmentid,employeesectionid,jobid,contractid,scheduleid,ismanager,hiringdate,employeeemail) values (?,?,?,?,?,?,?,?,?,?,?,?,?)';
      $query_prm = array($hourspermonth,$basesalary,$referencenumber,$employeename,$employeefirstname,$employeedepartmentid,$employeesectionid,$jobid,$contractid,$scheduleid,$ismanager,$hiringdate,$employeeemail);
      require('inc/doquery.php');

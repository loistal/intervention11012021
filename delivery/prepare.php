...
if ($showonly == 'Aperçu')
{
  ...
  <form id="TheForm" method="post" action="reportwindow.php" target="_blank">
  "Preview" affiché.
  <?php
  $ourcounter = 0;
...
if ($ok)
{
  $query = 'select invoiceid from invoicehistory where confirmed=1 and invoicegroupid=0 and isreturn=? and deliverydate>=? and deliverydate<=?';
  ...
  for ($i=0; $i < $num_results_main; $i++)
  {
    ...
    if (isset($_POST[$invoicenumber]) && $_POST[$invoicenumber] == 1)
    {
      if ($invoicegroupid == 0)
      {
        ...
        echo 'Livraison <a href="reportwindow.php?report=deliverylist&invoicegroupid=' . $invoicegroupid . '" target=_blank>' . $invoicegroupid . '</a> créé avec factures: ';
      }
      $query = 'update invoicehistory set invoicegroupid="' . $invoicegroupid . '" where invoiceid="' . $invoiceid . '"';
      $query_prm = array();
      require('inc/doquery.php');
      echo '<a href="printwindow.php?report=showinvoice&usedefaultstyle=1&invoiceid=' . $invoiceid . '" target=_blank>'.$invoiceid.'</a> ';
      $query = 'select invoiceweight from invoicehistory where invoiceid="' . $invoiceid . '"';
      $query_prm = array();
      require('inc/doquery.php');
      $row2 = $query_result[0];
...
,clientname,extraname,invoicehistory.clientid as clientid,townname,islandname,localvesselid,initials,emphasiscolor,clientsectorid,hascold
from invoicehistory,client,town,island,usertable,clientterm
where client.clienttermid=clientterm.clienttermid and invoicehistory.userid=usertable.userid and client.townid=town.townid and town.islandid=island.islandid and invoicehistory.clientid=client.clientid
and confirmed=1 and invoicegroupid=0 and deliverydate>=? and deliverydate<=?';
...
echo '"> <input type="submit" name="showonly" value="Aperçu"> <td id="sum" align=right></td></td></tr>'; # Afficher

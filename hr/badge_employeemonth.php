...
<input type=hidden name="report" value="badge_employeemonth"> 
...
if ($_SESSION['ds_ishrsuperuser'] || $_SESSION['ds_ismanager'])
{
  ...
  $query = 'select employeeid,badgetime,badgeuserid,badgeusername from badgelog where deleted=0 and badgetime is not null and badgedate=? order by employeeid,badgetime';
  ...
  for ($i=0; $i < $num_results; $i++)
  {
    ...
    if (isset($employee_teamidA[$query_result[$i]['employeeid']]) && $_SESSION['ds_ismanager'] == $employee_teamidA[$query_result[$i]['employeeid']]) { $ok = 1; } # ???
    ...
    {    
      if ($i == 0 || $query_result[$i]['employeeid'] != $query_result[($i-1)]['employeeid'])
      {
        ...
        else { echo '<tr><td><i>Employé non reconnu: '.$query_result[$i]['badgeuserid'].' '.d_output($query_result[$i]['badgeusername']).'</i>'; }
      }

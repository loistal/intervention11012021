...
$PA['invoiceid'] = 'uint';
$PA['pagenumber'] = 'uint';
$PA['linesperpage'] = 'uint';
require('inc/readpost.php');
if ($pagenumber == 0) { $pagenumber = 1; }
if ($linesperpage == 0) { $linesperpage = $_SESSION['ds_invoicelines']; }
...
{
  if ($_SESSION['ds_startyear'] > substr($accountingdate,0,4)) { exit; }
}
###
...
where invoiceitemhistory.productid=product.productid and product.regulationtypeid=regulationtype.regulationtypeid and invoiceid="' . $invoiceid . '"';
#$query .= ' order by invoiceitemid'; 2013 01 17 as asked in email: added regroupnumber (regulationtype)
...
{
  $change_prix_ht = 0;
  $quantity2[$i] = ""; $numberperunit[$i] = ""; $salesunit[$i] = ""; $productname[$i] = ""; $taxcode2[$i] = ""; $packaging[$i] = ""; $price2[$i] = ""; $priceperunit2[$i] = ""; $showregulation[$i] = ""; $RPT[$i] = ""; $RPI[$i] = ""; $RPTvat[$i] = ""; $RPIvat[$i] = "";
  $row = $main_result[$i];
  ...
  else
  {
    ...
    require('inc/doquery.php');
    if (isset($query_result[0]['lineproducttypeid']) && $query_result[0]['lineproducttypeid'] > 0)
    { $producttypeid = $query_result[0]['lineproducttypeid']; }
    else { $producttypeid = 0; }
  }
  ###
  ...
  if ($testvar == 0) { $quantity2[$i] = $quantity2[$i] / $numberperunit[$i]; $salesunit[$i] = $unittypeA[$row['unittypeid']]; }
  else
  {
    $salesunit[$i] = "unité";
...
echo '</div>';
echo '<div class="newalert">Tél. 40 543 543<br>
Comptabilité : 40 54 35 41 ou 42<br>
Fax : 444 777 (Gratuit)<font size=-1> - 40 543 540</font></div>';
...
if ($deliveryagentid == 0)
{
  if ($pagenumber == $totalpages)
  {
    ...
    {
      echo '<table border=1 cellspacing=0 cellpadding=0><tr><td>Taux&nbsp;TVA</td><td>Base&nbsp;HT</td><td>Montant&nbsp;TVA</td></tr>';
    ...
    echo '<table border=0><tr><td align=right><b>';
...
echo '</table>';
#if ($showregulationcolumn) { echo '<p>*tahiti+ ht / tahiti+ ttc / isles ht / isles ttc</p>'; }
...
echo '<img src="pics/logo.png" height="50">';
echo '</div>';
...
if ($islandname == 'Tahiti' || $islandname == 'Moorea')
{
  if ($totalpages == 1 || ($totalpages > 1 && $pagenumber == $totalpages))
  {
    ...
    .signme {
      position: absolute;';
      if (1==1||$_SESSION['ds_browser'] == 'Chrome')
      { echo 'left: 800px;
      top: 800px;'; }
      else { echo 'right: 160px;
      bottom: 0px;'; }

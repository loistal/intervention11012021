...
for ($y = 0; $y < $num_lines; $y++)
{
  ...
  if (floor($y / $linesperpage) == ($pagenumber - 1))
  {
    ...
    if ($template >= 6)
    {
      ...
      {
        $informationTable .= '<td style="text-align: left;">' . $productname; ///////////////
        if ($_SESSION['ds_useproductcode'] == 1)
        {
          $informationTable .= '(' . d_decode($row2['suppliercode']) . ')';
        }
        else
        {
      ...
      $informationTable .= '>' . $quantity . ' ' . $unittypename; ////////////////////////
      if ($_SESSION['ds_use_salesprice_mod'])
      {
    ...
    else
    {
      ...
      {
        $informationTable .= '<td class="numbers" colspan=2>' . myfix($bcp); ///////////////
      }
    }
    ...
    else
    {
      ...
      $informationTable .= '<td class="numbers">' . myfix($row2['linevat']); /////////////////////
      if ($_SESSION['ds_show_unittotalvat'])
...
if ($totalpages == 1 || $pagenumber == $totalpages)
{
  ...
  else
  {
    ...
    if ($totaltva > 0)
    {
      $informationTotalPages .= '<tr><td colspan=' . $colspan . '>TVA</td><td class="numbers">' . myfix($totaltva) . '</td></tr>'; ///////////////////// TVA
    }
...
require('custom/mvb_showinvoice_style.php');

require('custom/mvb_showinvoice_layout.php');
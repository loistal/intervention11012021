...
if (isset($_POST['saveme']) && $_POST['saveme'] == 1)
{
  ...
  $currentthemecolorid = $_POST['currentthemecolorid'];
  ...
  if ($num_results > 0)
  {
    $messages = '<p>Le nom de thème que vous avez entré existe déjà !</p>';
  }
  else
  {
    ...
    $query = 'INSERT INTO color_theme(themeid, themename, bgcolor, fgcolor, linkcolor, menucolor, alertcolor, infocolor, formcolor, tablecolor, inputcolor, menubordercolor, menufontcolor, tablecolor1, tablecolor2, hovercolor, usehovercolor, nbtablecolors, usetablecolorsub, tablecolorsub, userid) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
    $query_prm = array(
      $themeid,
      $theme_name,
      $bgcolor,
    ...
    if ($num_results > 0)
    {
      $messages = '<p>La palette "' . $theme_name . '" a bien été crée !</p>';
      $newthemecolorid = (int) $query_insert_id;
      ...
      if (isset($_POST['changelayoutandcolor']) && $_POST['changelayoutandcolor'] == 1)
      {
        ...
        if ($num_results > 0)
        {
          ...
          $_SESSION['currentthemecolorid'] = $currentthemecolorid;
          header("refresh:0;  url=options.php?optionsmenu=addcolors");
          exit;
        }
      }
    }
...
if (isset($_SESSION['refreshcolorpage']) && $_SESSION['refreshcolorpage'])
{
  ...
  print '<p>La palette de couleur que vous avez ajouté est maintenant la couleur de votre thème !</p>';
...
if (isset($_POST['colorthemeid']) || isset($currentthemecolorid))
{
  if (!empty($_POST['colorthemeid']))
  {
    ...
    $currentthemecolorid = $_POST['colorthemeid'];
  }
  else
  {
  ...
  $query = 'SELECT bgcolor, fgcolor, linkcolor, menucolor, alertcolor, infocolor, formcolor, tablecolor, inputcolor, menubordercolor, menufontcolor, tablecolor1, tablecolor2, hovercolor, tablecolorsub FROM color_theme WHERE themeid = ?';
  ...
  #We select all available color
  $query = 'SELECT colorname, colorcode FROM color order by colorname';
  $query_prm = array();
  require('inc/doquery.php');
  ...
  $query = 'SELECT themename, usehovercolor, nbtablecolors, usetablecolorsub  FROM color_theme WHERE themeid = ?';
  $query_prm = array($color_theme_id);
  require('inc/doquery.php');

  $color_theme_integer_parameters = $query_result[0];
  $theme_name = $color_theme_integer_parameters['themename'];
...
<?php } ?>
  ...
  <table class=transparent>
    <tr>
      <td style="width: 50%;">
        <h2>Créer à partir de la palette
          "<?php print $theme_name; ?>"</h2>
        ...
        <form method="post" action="options.php">
          <table>
            <tr>
              ...
              <td>
                <input type="text" name="themename" size="20" required>
              </td>
            </tr>
            ...
            <tr>
              ...
              <td colspan="2" align="center">
                <input type=hidden name="currentthemecolorid"
                  value="<?php echo $color_theme_id; ?>">
                ...
                <input type="hidden" name="saveme" value="1"> <input
                  type="hidden"
                  name="optionsmenu"
                  value="<?php echo $optionsmenu; ?>"> <input type="submit"
...
else
{
  ...
  ?>
  <h2>Créer une nouvelle palette à partir d'une palette existante</h2>
  <form method="post" action="options.php">
    <table>
      <tr>
        <td>Palettes de couleurs disponibles</td>
        <td>
      ...
      <tr>
        <td colspan="2" align="center">
          <input type=hidden name="optionsmenu" value="addcolors"> <input
            type="submit" value="<?php echo d_trad('validate'); ?>">
        </td>
      </tr>
    </table>
  </form>
